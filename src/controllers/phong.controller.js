...
exports.ChitietPhong=(req,res)=>{
    db.query("SELECT p.MaPhong,p.TenPhong,p.TinhTrang AS TinhTrangPhong, kh.HoTen AS TenKhachHang,kh.SDT,tb.TenTB AS TenThietBi,tb.SoSeri, tbp.TinhTrang AS TinhTrangThietBi FROM phong p LEFT JOIN hopdong hd ON p.MaPhong = hd.MaPhong LEFT JOIN khachhang kh ON hd.MaKH = kh.MaKH LEFT JOIN thietbiphong tbp ON p.MaPhong = tbp.MaPhong LEFT JOIN thietbi tb ON tbp.MaTB = tb.MaTB WHERE p.MaPhong = ?",[req.params.id],(err,result)=>{
        if(err)
            {
                console.error("Lỗi truy vấn cơ sở dữ liệu:", err);
                res.status(500).json({ error: "Lỗi truy vấn cơ sở dữ liệu" });
